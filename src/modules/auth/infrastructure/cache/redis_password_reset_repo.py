...
class RedisPasswordResetRepository(IPasswordResetRepository):
    ...
    async def add(self, token: str, user_id: ID, ttl_seconds: int = 900) -> None:
        logger.info("storing user's password reset token: user_id=%s, TTL=%ss", user_id.value, ttl_seconds)
        key = f"{self.KEY_PREFIX}{token}"
        await self._execute_redis_operation(
            "store_user_password_reset _token",
            self._client.setex,
            key, ttl_seconds, user_id.value
        )
        logger.info("Password reset token stored successfully: user_id=%s, TTL=%ss", user_id.value, ttl_seconds)
    ...
    async def get(self, token: str) -> ID | None:
        ...
        user_id = await self._execute_redis_operation(
            "get_pass_reset_token",
            self._client.get,
            key
        )
        
        if user_id is None:
            logger.debug("Password reset token not found or expired: token=%s...", token[:8])
            return None
        
        logger.debug(f"Password reset token found successfully: user_id=%s, token=%s...", user_id, token[:8])
        return ID(user_id)
