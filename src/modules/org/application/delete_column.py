...
class DeleteColumnService:
    ...
    async def execute(self, access_token: str, column_id: str) -> None:
        logger.info("Deleting column: column_id=%s", column_id)
        ...
        if user_role is None or user_role.value not in {"owner", "admin"}:
            ...
            raise PermissionDenied("Only owner or admin can delete columns")

        # Delete column
        await self.uow.columns.delete(column_id_vo)
        await self.uow.commit()

        logger.info("Column deleted successfully: column_id=%s", column_id)
