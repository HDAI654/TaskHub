...
class CreateBoardService:
    ...
    async def execute(
        self, access_token: str, project_id: str, name: str, description: str
    ) -> BoardEntity:
        logger.info("Creating board: name=%s, project_id=%s", name, project_id)
        ...
        if user_role is None or user_role.value not in {"owner", "admin"}:
            ...
            raise PermissionDenied("Only owner or admin can create boards")
        ...
        await self.uow.boards.add(board)
        await self.uow.commit()

        logger.info(
            "Board created successfully: board_id=%s, project_id=%s",
            board.id.value,
            project_id,
        )
        return board
