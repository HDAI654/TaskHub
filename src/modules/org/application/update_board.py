...
class UpdateBoardService:
    ...
    async def execute(
        ...
        logger.info(
            "Updating board: board_id=%s, new_name=%s, new_description=%s",
            board_id,
            new_name,
            new_description,
        )
        ...
        if user_role is None or user_role.value not in {"owner", "admin"}:
            ...
            raise PermissionDenied("Only owner or admin can edit boards")

        # Update board
        name_vo = Name(new_name) if new_name is not None else None
        desc_vo = Description(new_description) if new_description is not None else None
        await self.uow.boards.update(board_id_vo, name_vo, desc_vo)
        await self.uow.commit()

        logger.info("Board updated successfully: board_id=%s", board_id)
