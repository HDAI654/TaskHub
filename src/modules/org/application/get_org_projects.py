...
class GetOrgProjectsService:
    ...
    async def execute(self, access_token: str, org_id: str) -> List[PrjEntity]:
        logger.info("Getting projects for organization: org_id=%s", org_id)
        ...
        # Get projects
        projects = await self.uow.projects.get_by_org_id(org_id_vo)

        logger.info(
            "Projects retrieved successfully: org_id=%s, count=%d",
            org_id,
            len(projects),
        )
        return projects
