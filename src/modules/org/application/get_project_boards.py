...
class GetProjectBoardsService:
    ...
    async def execute(self, access_token: str, project_id: str) -> List[BoardEntity]:
        logger.info("Getting boards for project: project_id=%s", project_id)
        ...
        # Get boards
        boards = await self.uow.boards.get_by_prj_id(project_id_vo)

        logger.info(
            "Boards retrieved successfully: project_id=%s, count=%d",
            project_id,
            len(boards),
        )
        return boards
