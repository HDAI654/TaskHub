...
class GetBoardColumnsService:
    ...
    async def execute(self, access_token: str, board_id: str) -> List[ColumnEntity]:
        logger.info("Getting columns for board: board_id=%s", board_id)
        ...
        columns = await self.uow.columns.get_by_board_id(board_id_vo)

        logger.info(
            "Columns retrieved successfully: board_id=%s, count=%d",
            board_id,
            len(columns),
        )
        return columns
