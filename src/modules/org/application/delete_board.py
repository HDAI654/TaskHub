...
class DeleteBoardService:
    ...
    async def execute(self, access_token: str, board_id: str) -> None:
        logger.info("Deleting board: board_id=%s", board_id)
        ...
        if user_role is None or user_role.value not in {"owner", "admin"}:
            ...
            raise PermissionDenied("Only owner or admin can delete boards")

        # Delete board
        await self.uow.boards.delete(board_id_vo)
        await self.uow.commit()

        logger.info("Board deleted successfully: board_id=%s", board_id)
