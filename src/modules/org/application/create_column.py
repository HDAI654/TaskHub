...
class CreateColumnService:
    ...
    async def execute(
        self, access_token: str, board_id: str, name: str, order: int
    ) -> ColumnEntity:
        logger.info(
            "Creating column: name=%s, board_id=%s, order=%d", name, board_id, order
        )
        ...
        if user_role is None or user_role.value not in {"owner", "admin"}:
            ...
            raise PermissionDenied("Only owner or admin can create columns")
        ...
        await self.uow.columns.add(column)
        await self.uow.commit()

        logger.info(
            "Column created successfully: column_id=%s, board_id=%s, order=%d",
            column.id.value,
            board_id,
            order,
        )
