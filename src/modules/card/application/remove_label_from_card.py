...
class RemoveLabelFromCardService:
    def __init__(
        self,
        uow: IUnitOfWork,
    ...
    async def execute(self, access_token: str, card_id: str, label_name: str) -> None:
        logger.info(
            "Removing label from card: card_id=%s, label_name=%s", card_id, label_name
        )
        ...
        if role is None or role.value not in {"owner", "admin"}:
            logger.debug(
                "User lacks permission to remove labels: user_id=%s, org_id=%s, role=%s",
                current_user_id.value,
                org_id.value,
                role.value if role else None,
            )
            raise PermissionDenied("Only owner or admin can remove labels")

        label = LabelFactory.create(name=label_name)
        await self.uow.cards.del_label(card_id_vo, label)
        await self.uow.commit()

        logger.info(
            "Label removed from card: card_id=%s, label_name=%s", card_id, label_name
        )
