...
class AddAssigneeService:
    ...
    async def execute(self, access_token: str, card_id: str, assignee_id: str) -> None:
        logger.info(
            "Adding assignee to card: card_id=%s, assignee_id=%s",
            card_id,
            assignee_id,
        )
        ...
        if role is None or role.value not in {"owner", "admin"}:
            logger.debug(
                "User lacks permission to add assignees: user_id=%s, org_id=%s, role=%s",
                current_user_id.value,
                org_id.value,
                role.value if role else None,
            )
            raise PermissionDenied("Only owner or admin can add assignees")
        ...
        try:
            assignee = await self.uow.users.get_by_id(assignee_id_vo)
        except UserNotFoundError:
            logger.warning("Assignee user not found: user_id=%s", assignee_id)
            raise

        await self.uow.cards.add_assignee(card_id_vo, assignee)
        await self.uow.commit()

        logger.info(
            "Assignee added successfully: card_id=%s, assignee_id=%s",
            card_id,
            assignee_id,
        )
