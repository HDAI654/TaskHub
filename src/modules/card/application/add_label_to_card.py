...
class AddLabelToCardService:
    ...
    async def execute(self, access_token: str, card_id: str, label_name: str) -> None:
        logger.info(
            "Adding label to card: card_id=%s, label_name=%s", card_id, label_name
        )
        ...
        if role is None or role.value not in {"owner", "admin"}:
            logger.debug(
                "User lacks permission to add labels: user_id=%s, org_id=%s, role=%s",
                current_user_id.value,
                org_id.value,
                role.value if role else None,
            )
            raise PermissionDenied("Only owner or admin can add labels")

        # Create label entity
        label = LabelFactory.create(name=label_name)

        await self.uow.cards.add_label(card_id_vo, label)
        await self.uow.commit()

        logger.info(
            "Label added to card: card_id=%s, label_name=%s", card_id, label_name
        )
