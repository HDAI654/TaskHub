...
class GetCardChecklistsService:
    def __init__(
        self,
        uow: IUnitOfWork,
    ...
    async def execute(self, access_token: str, card_id: str) -> List[CheckListEntity]:
        logger.info("Getting checklists for card: card_id=%s", card_id)
        ...
        checklists = await self.uow.cards.get_card_checklists(card_id_vo)

        logger.info(
            "Found %d checklists for card: card_id=%s", len(checklists), card_id
        )
        return checklists
