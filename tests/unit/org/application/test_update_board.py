...
class TestUpdateBoard:
    ...
    @pytest.fixture
    async def update_board_service(self, uow, token_repo, decoder):
        return UpdateBoardService(uow, token_repo, decoder)
    ...
    async def sample_board(
        ...
        board = await create_board_service.execute(
            access_token, test_project.id.value, "Original Board", "Original Desc"
        )
        return board

    async def test_update_board_name_success_by_owner(
        self, owner_user, sample_board, update_board_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        new_name = "Updated Board Name"
        await update_board_service.execute(
            access_token, sample_board.id.value, new_name=new_name
        )
        updated = await update_board_service.uow.boards.get_by_id(sample_board.id)
        assert updated.name.value == new_name
        assert updated.description.value == "Original Desc"

    async def test_update_board_desc_success_by_admin(
        self, admin_user, sample_board, update_board_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=admin_user.id)
        new_desc = "Admin Updated Desc"
        await update_board_service.execute(
            access_token, sample_board.id.value, new_description=new_desc
        )
        updated = await update_board_service.uow.boards.get_by_id(sample_board.id)
        assert updated.description.value == new_desc
        assert updated.name.value == "Original Board"

    async def test_update_board_both_success_by_owner(
        self, owner_user, sample_board, update_board_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        new_name = "Both Name"
        new_desc = "Both Desc"
        await update_board_service.execute(
            access_token, sample_board.id.value, new_name, new_desc
        )
        updated = await update_board_service.uow.boards.get_by_id(sample_board.id)
        assert updated.name.value == new_name
        assert updated.description.value == new_desc

    async def test_update_board_permission_denied_for_member(
        self, member_user, sample_board, update_board_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=member_user.id)
        with pytest.raises(PermissionDenied):
            await update_board_service.execute(
                access_token, sample_board.id.value, new_name="Hack"
            )

    async def test_update_board_not_found(
        self, owner_user, update_board_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
    ...
    async def test_update_board_invalid_token(self, update_board_service, sample_board):
        with pytest.raises(InvalidToken):
            await update_board_service.execute(
                "invalid_token", sample_board.id.value, new_name="New"
            )
