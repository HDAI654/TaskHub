...
class TestDeleteColumn:
    ...
    @pytest.fixture
    async def delete_column_service(self, uow, token_repo, decoder):
        return DeleteColumnService(uow, token_repo, decoder)
    ...
    async def sample_column(
        ...
        column = await create_column_service.execute(
            access_token, test_board.id.value, "To Delete", 1
        )
        return column

    async def test_delete_column_success_by_owner(
        self, owner_user, sample_column, delete_column_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        await delete_column_service.execute(access_token, sample_column.id.value)
        with pytest.raises(ColumnNotFoundError):
            await delete_column_service.uow.columns.get_by_id(sample_column.id)

    async def test_delete_column_success_by_admin(
        self, admin_user, sample_column, delete_column_service, encoder
    ):
    ...
    async def test_delete_column_permission_denied_for_member(
        self, member_user, sample_column, delete_column_service, encoder
    ):
    ...
    async def test_delete_column_not_found(
        self, owner_user, delete_column_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        with pytest.raises(ColumnNotFoundError):
            await delete_column_service.execute(access_token, "non-existent-id")

    async def test_delete_column_invalid_token(
        self, delete_column_service, sample_column
    ):
        with pytest.raises(InvalidToken):
            await delete_column_service.execute("invalid_token", sample_column.id.value)
