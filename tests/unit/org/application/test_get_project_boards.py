...
class TestGetProjectBoards:
    ...
    @pytest.fixture
    async def get_project_boards_service(self, uow, token_repo, decoder):
        return GetProjectBoardsService(uow, token_repo, decoder)

    @pytest.fixture
    async def sample_boards(
        self, owner_user, test_project, create_board_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        board1 = await create_board_service.execute(
            access_token, test_project.id.value, "Board 1", "Desc1"
        )
        board2 = await create_board_service.execute(
            access_token, test_project.id.value, "Board 2", "Desc2"
        )
        return [board1, board2]

    async def test_get_project_boards_success_by_owner(
        self,
        owner_user,
        test_project,
        sample_boards,
        get_project_boards_service,
        encoder,
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        boards = await get_project_boards_service.execute(
            access_token, test_project.id.value
        )
        assert len(boards) == 2

    async def test_get_project_boards_success_by_member(
        self,
        member_user,
        test_project,
        sample_boards,
    ...
    async def test_get_project_boards_permission_denied_for_non_member(
        self, non_member_user, test_project, get_project_boards_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=non_member_user.id)
        with pytest.raises(PermissionDenied):
            await get_project_boards_service.execute(
                access_token, test_project.id.value
            )

    async def test_get_project_boards_project_not_found(
        self, owner_user, get_project_boards_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        with pytest.raises(ProjectNotFoundError):
            await get_project_boards_service.execute(access_token, "non-existent-id")

    async def test_get_project_boards_invalid_token(
        self, get_project_boards_service, test_project
    ):
        with pytest.raises(InvalidToken):
            await get_project_boards_service.execute(
                "invalid_token", test_project.id.value
            )
