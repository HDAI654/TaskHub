...
class TestCreateColumn:
    ...
    async def test_create_column_success_by_owner(
        self, owner_user, test_board, create_column_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        column = await create_column_service.execute(
            access_token, test_board.id.value, "Owner Column", 1
        )
        assert column.name.value == "Owner Column"
        assert column.order.value == 1
        assert column.board_id.value == test_board.id.value

    async def test_create_column_success_by_admin(
        self, admin_user, test_board, create_column_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=admin_user.id)
        column = await create_column_service.execute(
            access_token, test_board.id.value, "Admin Column", 2
        )
        assert column.name.value == "Admin Column"
        assert column.board_id.value == test_board.id.value

    async def test_create_column_permission_denied_for_member(
        self, member_user, test_board, create_column_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=member_user.id)
        with pytest.raises(PermissionDenied):
            await create_column_service.execute(
                access_token, test_board.id.value, "Member Column", 3
            )

    async def test_create_column_board_not_found(
        self, owner_user, create_column_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        with pytest.raises(BoardNotFoundError):
            await create_column_service.execute(
                access_token, "non-existent-id", "Column", 1
            )

    async def test_create_column_invalid_token(self, create_column_service, test_board):
        with pytest.raises(InvalidToken):
            await create_column_service.execute(
                "invalid_token", test_board.id.value, "Column", 1
            )
