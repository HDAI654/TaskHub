...
class TestCreateBoard:
    ...
    async def test_create_board_success_by_owner(
        self, owner_user, test_project, create_board_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        board = await create_board_service.execute(
            access_token, test_project.id.value, "Owner Board", "Board Desc"
        )
        assert board.name.value == "Owner Board"
        assert board.description.value == "Board Desc"
        assert board.prj_id.value == test_project.id.value

    async def test_create_board_success_by_admin(
        self, admin_user, test_project, create_board_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=admin_user.id)
        board = await create_board_service.execute(
            access_token, test_project.id.value, "Admin Board", "Admin Desc"
        )
        assert board.name.value == "Admin Board"
        assert board.prj_id.value == test_project.id.value

    async def test_create_board_permission_denied_for_member(
        self, member_user, test_project, create_board_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=member_user.id)
        with pytest.raises(PermissionDenied):
            await create_board_service.execute(
                access_token, test_project.id.value, "Member Board", "Desc"
            )

    async def test_create_board_project_not_found(
        self, owner_user, create_board_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        with pytest.raises(ProjectNotFoundError):
            await create_board_service.execute(
                access_token, "non-existent-id", "Board", "Desc"
            )

    async def test_create_board_invalid_token(self, create_board_service, test_project):
        with pytest.raises(InvalidToken):
            await create_board_service.execute(
                "invalid_token", test_project.id.value, "Board", "Desc"
            )
