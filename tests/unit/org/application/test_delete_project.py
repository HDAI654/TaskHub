...
class TestDeleteProject:
    ...
    @pytest.fixture
    async def delete_project_service(self, uow, token_repo, decoder):
        return DeleteProjectService(uow, token_repo, decoder)
    ...
    async def sample_project(
        ...
        project = await create_project_service.execute(
            access_token, test_org.id.value, "To Delete", "Desc"
        )
        return project

    async def test_delete_project_success_by_owner(
        self, owner_user, sample_project, delete_project_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        await delete_project_service.execute(access_token, sample_project.id.value)
        with pytest.raises(ProjectNotFoundError):
            await delete_project_service.uow.projects.get_by_id(sample_project.id)

    async def test_delete_project_success_by_admin(
        self, admin_user, sample_project, delete_project_service, encoder
    ):
    ...
    async def test_delete_project_permission_denied_for_member(
        self, member_user, sample_project, delete_project_service, encoder
    ):
    ...
    async def test_delete_project_not_found(
        self, owner_user, delete_project_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        with pytest.raises(ProjectNotFoundError):
            await delete_project_service.execute(access_token, "non-existent-id")

    async def test_delete_project_invalid_token(
        self, delete_project_service, sample_project
    ):
        with pytest.raises(InvalidToken):
            await delete_project_service.execute(
                "invalid_token", sample_project.id.value
            )
