...
class TestGetBoard:
    ...
    @pytest.fixture
    async def get_board_service(self, uow, token_repo, decoder):
        return GetBoardService(uow, token_repo, decoder)
    ...
    async def test_get_board_success_by_owner(
        self, owner_user, sample_board, get_board_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        board = await get_board_service.execute(access_token, sample_board.id.value)
        assert board.id.value == sample_board.id.value

    async def test_get_board_success_by_member(
        self, member_user, sample_board, get_board_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=member_user.id)
        board = await get_board_service.execute(access_token, sample_board.id.value)
        assert board.id.value == sample_board.id.value

    async def test_get_board_permission_denied_for_non_member(
        self, non_member_user, sample_board, get_board_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=non_member_user.id)
        with pytest.raises(PermissionDenied):
            await get_board_service.execute(access_token, sample_board.id.value)

    async def test_get_board_not_found(self, owner_user, get_board_service, encoder):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        with pytest.raises(BoardNotFoundError):
            await get_board_service.execute(access_token, "non-existent-id")

    async def test_get_board_invalid_token(self, get_board_service, sample_board):
        with pytest.raises(InvalidToken):
            await get_board_service.execute("invalid_token", sample_board.id.value)
