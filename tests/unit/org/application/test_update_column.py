...
class TestUpdateColumn:
    ...
    async def test_board(self, owner_user, test_project, create_board_service, encoder):
        ...
        board = await create_board_service.execute(
            access_token, test_project.id.value, "Test Board", "Board Desc"
        )
        return board
    ...
    @pytest.fixture
    async def update_column_service(self, uow, token_repo, decoder):
        return UpdateColumnService(uow, token_repo, decoder)
    ...
    async def sample_column(
        ...
        column = await create_column_service.execute(
            access_token, test_board.id.value, "Original Column", 1
        )
        return column

    async def test_update_column_name_success_by_owner(
        self, owner_user, sample_column, update_column_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        new_name = "Updated Column Name"
        await update_column_service.execute(
            access_token, sample_column.id.value, new_name=new_name
        )
        updated = await update_column_service.uow.columns.get_by_id(sample_column.id)
        assert updated.name.value == new_name
        assert updated.order.value == sample_column.order.value

    async def test_update_column_order_success_by_admin(
        self, admin_user, sample_column, update_column_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=admin_user.id)
        new_order = 5
        await update_column_service.execute(
            access_token, sample_column.id.value, new_order=new_order
        )
        updated = await update_column_service.uow.columns.get_by_id(sample_column.id)
        assert updated.order.value == new_order
        assert updated.name.value == sample_column.name.value

    async def test_update_column_both_success_by_owner(
        self, owner_user, sample_column, update_column_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        new_name = "Both Name"
        new_order = 10
        await update_column_service.execute(
            access_token, sample_column.id.value, new_name, new_order
        )
        updated = await update_column_service.uow.columns.get_by_id(sample_column.id)
        assert updated.name.value == new_name
        assert updated.order.value == new_order

    async def test_update_column_permission_denied_for_member(
        self, member_user, sample_column, update_column_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=member_user.id)
        with pytest.raises(PermissionDenied):
            await update_column_service.execute(
                access_token, sample_column.id.value, new_name="Hack"
            )

    async def test_update_column_not_found(
        self, owner_user, update_column_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
    ...
    async def test_update_column_invalid_token(
        self, update_column_service, sample_column
    ):
        with pytest.raises(InvalidToken):
            await update_column_service.execute(
                "invalid_token", sample_column.id.value, new_name="New"
            )
