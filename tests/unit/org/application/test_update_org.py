...
class TestUpdateOrg:
    ...
    @pytest.fixture
    async def update_service(self, uow, token_repo, decoder):
        return UpdateOrgService(uow, token_repo, decoder)
    ...
    async def test_update_org_success(self, user, update_service, encoder, org):
        access_token = encoder.create_access_token(user_id=user.id)
        new_name = "Updated Org Name"
        await update_service.execute(access_token, org.id.value, new_name)
        updated_org = await update_service.uow.orgs.get_by_id(org.id)
        assert updated_org.name.value == new_name

    async def test_update_org_not_owner(
        self, second_user, update_service, encoder, org
    ):
        access_token = encoder.create_access_token(user_id=second_user.id)
        with pytest.raises(PermissionDenied):
            await update_service.execute(access_token, org.id.value, "New Name")

    async def test_update_org_not_found(self, user, update_service, encoder):
        access_token = encoder.create_access_token(user_id=user.id)
        with pytest.raises(OrgNotFoundError):
            await update_service.execute(access_token, ID().value, "New Name")
