...
class TestDelAcc:
    ...
    async def test_del_acc_success(
        self, user, service, encoder, token_repo: RedisTokenRepository, decoder
    ):
        ...
        assert await token_repo.get_user_version(user_id=user.id) == 1

    async def test_del_acc_with_invalid_version(self, service, user, encoder):
