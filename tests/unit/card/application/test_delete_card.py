...
class TestDeleteCard:
    ...
    @pytest.fixture
    async def delete_card_service(self, uow, token_repo, decoder):
        return DeleteCardService(uow, token_repo, decoder)
    ...
    @pytest.fixture
    async def admin_card(self, admin_user, test_column, create_card_service, encoder):
    ...
    async def test_delete_card_success_by_owner(
        self, owner_user, sample_card, delete_card_service, encoder
    ):
        """Owner should be able to delete a card."""
        access_token = encoder.create_access_token(user_id=owner_user.id)

        await delete_card_service.execute(access_token, sample_card.id.value)

        # Verify card is deleted
        with pytest.raises(CardNotFoundError):
            await delete_card_service.uow.cards.get_by_id(sample_card.id)

    async def test_delete_card_success_by_admin(
        self, admin_user, sample_card, delete_card_service, encoder
    ):
        """Admin should be able to delete any card in the organization."""
        access_token = encoder.create_access_token(user_id=admin_user.id)
    ...
    async def test_delete_card_created_by_admin_deleted_by_owner(
        self, owner_user, admin_card, delete_card_service, encoder
    ):
        """Owner should be able to delete cards created by admin."""
        access_token = encoder.create_access_token(user_id=owner_user.id)

        await delete_card_service.execute(access_token, admin_card.id.value)

        # Verify card is deleted
        with pytest.raises(CardNotFoundError):
            await delete_card_service.uow.cards.get_by_id(admin_card.id)

    async def test_delete_card_permission_denied_for_member(
        self, member_user, sample_card, delete_card_service, encoder
    ):
        """Member should not be able to delete cards."""
        access_token = encoder.create_access_token(user_id=member_user.id)

        with pytest.raises(PermissionDenied) as exc_info:
            await delete_card_service.execute(access_token, sample_card.id.value)
        assert "Only owner or admin can delete cards" in str(exc_info.value)

        # Verify card still exists
        card = await delete_card_service.uow.cards.get_by_id(sample_card.id)
        assert card is not None

    async def test_delete_card_not_found(
        self, owner_user, delete_card_service, encoder
    ):
        """Should raise CardNotFoundError when card doesn't exist."""
        access_token = encoder.create_access_token(user_id=owner_user.id)

        with pytest.raises(CardNotFoundError):
            await delete_card_service.execute(access_token, ID().value)

    async def test_delete_card_invalid_token(self, sample_card, delete_card_service):
        """Should raise InvalidToken when token is invalid."""
        with pytest.raises(InvalidToken):
            await delete_card_service.execute("invalid_token", sample_card.id.value)

    async def test_delete_card_token_version_mismatch(
        self, owner_user, sample_card, delete_card_service, encoder, token_repo
    ):
        """Should raise InvalidToken when token version doesn't match."""
        access_token = encoder.create_access_token(user_id=owner_user.id)
