...
class TestDeleteChecklist:
    ...
    @pytest.fixture
    async def delete_checklist_service(self, uow, token_repo, decoder):
        return DeleteChecklistService(uow, token_repo, decoder)
    ...
    async def sample_checklist(
        ...
        await add_checklist_service.execute(
            access_token, sample_card.id.value, "To Delete"
        )
    ...
    async def test_delete_checklist_success_by_owner(
        self,
        owner_user,
        sample_checklist,
        sample_card,
        ...
        access_token = encoder.create_access_token(user_id=owner_user.id)
        await delete_checklist_service.execute(access_token, sample_checklist.id.value)

        # Verify checklist is deleted
        with pytest.raises(CheckListNotFoundError):
            await delete_checklist_service.uow.cards.get_checklist_by_id(
                sample_checklist.id
            )

        # Verify card still exists
        card = await delete_checklist_service.uow.cards.get_by_id(sample_card.id)
        assert card.id.value == sample_card.id.value

    async def test_delete_checklist_success_by_admin(
        self, admin_user, sample_checklist, delete_checklist_service, encoder
    ):
    ...
    async def test_delete_checklist_permission_denied_for_member(
        self, member_user, sample_checklist, delete_checklist_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=member_user.id)
        with pytest.raises(PermissionDenied) as exc_info:
            await delete_checklist_service.execute(
                access_token, sample_checklist.id.value
            )
        assert "Only owner or admin can delete checklists" in str(exc_info.value)

        # Verify checklist still exists
        checklist = await delete_checklist_service.uow.cards.get_checklist_by_id(
            sample_checklist.id
        )
        assert checklist.id.value == sample_checklist.id.value

    async def test_delete_checklist_permission_denied_for_viewer(
        self, viewer_user, sample_checklist, delete_checklist_service, encoder
    ):
    ...
    async def test_delete_checklist_permission_denied_for_non_member(
        self, non_member_user, sample_checklist, delete_checklist_service, encoder
    ):
    ...
    async def test_delete_checklist_not_found(
        self, owner_user, delete_checklist_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        with pytest.raises(CheckListNotFoundError):
            await delete_checklist_service.execute(access_token, ID().value)

    async def test_delete_checklist_invalid_id_format(
        self, owner_user, delete_checklist_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        with pytest.raises(CheckListNotFoundError):
            await delete_checklist_service.execute(access_token, "invalid-id-format")
