...
class TestGetColumnCards:
    ...
    @pytest.fixture
    async def get_column_cards_service(self, uow, token_repo, decoder):
        return GetColumnCardsService(uow, token_repo, decoder)

    @pytest.fixture
    async def sample_cards(self, owner_user, test_column, create_card_service, encoder):
    ...
    async def test_get_column_cards_success_by_owner(
        self, owner_user, test_column, sample_cards, get_column_cards_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        cards = await get_column_cards_service.execute(
            access_token, test_column.id.value
        )
        assert len(cards) == 3
        titles = [c.title.value for c in cards]
        assert "Card 1" in titles
        assert "Card 2" in titles
        assert "Card 3" in titles

    async def test_get_column_cards_success_by_admin(
        self, admin_user, test_column, sample_cards, get_column_cards_service, encoder
    ):
    ...
    async def test_get_column_cards_success_by_member(
        self, member_user, test_column, sample_cards, get_column_cards_service, encoder
    ):
    ...
    async def test_get_column_cards_permission_denied_for_non_member(
        self, non_member_user, test_column, get_column_cards_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=non_member_user.id)
        with pytest.raises(PermissionDenied) as exc_info:
            await get_column_cards_service.execute(access_token, test_column.id.value)
        assert "You do not have access to this column" in str(exc_info.value)

    async def test_get_column_cards_empty_column(
        self, owner_user, test_column, get_column_cards_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        cards = await get_column_cards_service.execute(
            access_token, test_column.id.value
    ...
    async def test_get_column_cards_column_not_found(
        self, owner_user, get_column_cards_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        with pytest.raises(ColumnNotFoundError):
            await get_column_cards_service.execute(access_token, ID().value)

    async def test_get_column_cards_invalid_column_id(
        self, owner_user, get_column_cards_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        with pytest.raises(ColumnNotFoundError):
            await get_column_cards_service.execute(access_token, "invalid-id-format")
