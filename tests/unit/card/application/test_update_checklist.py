...
class TestUpdateChecklist:
    ...
    @pytest.fixture
    async def update_checklist_service(self, uow, token_repo, decoder):
        return UpdateChecklistService(uow, token_repo, decoder)
    ...
    async def sample_checklist(
        ...
        await add_checklist_service.execute(
            access_token, sample_card.id.value, "Original Title"
        )
    ...
    async def test_update_checklist_title_by_owner(
        self, owner_user, sample_checklist, update_checklist_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        new_title = "Updated Title by Owner"
        await update_checklist_service.execute(
            access_token, sample_checklist.id.value, new_title=new_title
        )

        updated = await update_checklist_service.uow.cards.get_checklist_by_id(
            sample_checklist.id
        )
        assert updated.title.value == new_title
        assert updated.is_checked.value is False  # unchanged

    async def test_update_checklist_is_checked_by_admin(
        self, admin_user, sample_checklist, update_checklist_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=admin_user.id)
        await update_checklist_service.execute(
            access_token, sample_checklist.id.value, new_is_checked=True
        )
        ...
        assert updated.title.value == "Original Title"  # unchanged

    async def test_update_checklist_both_by_member(
        self, member_user, sample_checklist, update_checklist_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=member_user.id)
        new_title = "Member Updated Title"
        await update_checklist_service.execute(
            access_token,
            sample_checklist.id.value,
            new_title=new_title,
            new_is_checked=True,
        )
    ...
    async def test_update_checklist_permission_denied_for_viewer(
        self, viewer_user, sample_checklist, update_checklist_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=viewer_user.id)
        with pytest.raises(PermissionDenied) as exc_info:
            await update_checklist_service.execute(
                access_token, sample_checklist.id.value, new_title="Viewer Title"
            )
        assert "Only owner, admin, or member can update checklists" in str(
            exc_info.value
        )

    async def test_update_checklist_permission_denied_for_non_member(
        self, non_member_user, sample_checklist, update_checklist_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=non_member_user.id)
        with pytest.raises(PermissionDenied) as exc_info:
            await update_checklist_service.execute(
                access_token, sample_checklist.id.value, new_title="Hack"
            )
        assert "Only owner, admin, or member can update checklists" in str(
            exc_info.value
        )
    ...
    async def test_update_checklist_not_found(
        self, owner_user, update_checklist_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        with pytest.raises(CheckListNotFoundError):
            await update_checklist_service.execute(
                access_token, ID().value, new_title="New Title"
            )

    async def test_update_checklist_no_changes(
        self, owner_user, sample_checklist, update_checklist_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        with pytest.raises(NoChangesError):
            await update_checklist_service.execute(
                access_token, sample_checklist.id.value
            )
