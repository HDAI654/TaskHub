...
class TestGetCardById:
    ...
    async def non_member_user(self, uow, user_hashed_password):
        """User that is not a member of the organization."""
        user = UserFactory.create(
    ...
    @pytest.fixture
    async def get_card_by_id_service(self, uow, token_repo, decoder):
        return GetCardByIdService(uow, token_repo, decoder)
    ...
    async def test_get_card_by_id_success_by_owner(
        self, owner_user, sample_card, get_card_by_id_service, encoder
    ):
        """Owner should be able to view the card."""
        access_token = encoder.create_access_token(user_id=owner_user.id)

        card = await get_card_by_id_service.execute(access_token, sample_card.id.value)

        assert card.id.value == sample_card.id.value
        assert card.title.value == "Test Card"
        assert card.description.value == "Test Description"
        assert card.priority.value == "high"
        assert card.column_id.value == sample_card.column_id.value
        assert card.created_by_user_id.value == owner_user.id.value

    async def test_get_card_by_id_success_by_admin(
        self, admin_user, sample_card, get_card_by_id_service, encoder
    ):
        """Admin should be able to view the card."""
    ...
    async def test_get_card_by_id_success_by_member(
        self, member_user, sample_card, get_card_by_id_service, encoder
    ):
        """Member should be able to view the card (everyone in org has access)."""
    ...
    async def test_get_card_by_id_permission_denied_for_non_member(
        self, non_member_user, sample_card, get_card_by_id_service, encoder
    ):
        """Non-member should not be able to view the card."""
        access_token = encoder.create_access_token(user_id=non_member_user.id)

        with pytest.raises(PermissionDenied) as exc_info:
            await get_card_by_id_service.execute(access_token, sample_card.id.value)
        assert "You do not have access to this card" in str(exc_info.value)

    async def test_get_card_by_id_not_found(
        self, owner_user, get_card_by_id_service, encoder
    ):
        """Should raise CardNotFoundError when card doesn't exist."""
        access_token = encoder.create_access_token(user_id=owner_user.id)

        with pytest.raises(CardNotFoundError):
            await get_card_by_id_service.execute(access_token, ID().value)
