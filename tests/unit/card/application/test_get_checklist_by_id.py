...
class TestGetChecklistById:
    ...
    @pytest.fixture
    async def get_checklist_by_id_service(self, uow, token_repo, decoder):
        return GetChecklistByIdService(uow, token_repo, decoder)
    ...
    async def test_get_checklist_by_id_success_by_owner(
        self, owner_user, sample_checklist, get_checklist_by_id_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        checklist = await get_checklist_by_id_service.execute(
            access_token, sample_checklist.id.value
        )
        assert checklist.id.value == sample_checklist.id.value
        assert checklist.title.value == "Test Checklist"
        assert checklist.card_id.value == sample_checklist.card_id.value

    async def test_get_checklist_by_id_success_by_admin(
        self, admin_user, sample_checklist, get_checklist_by_id_service, encoder
    ):
    ...
    async def test_get_checklist_by_id_success_by_member(
        self, member_user, sample_checklist, get_checklist_by_id_service, encoder
    ):
    ...
    async def test_get_checklist_by_id_success_by_viewer(
        self, viewer_user, sample_checklist, get_checklist_by_id_service, encoder
    ):
    ...
    async def test_get_checklist_by_id_permission_denied_for_non_member(
        self, non_member_user, sample_checklist, get_checklist_by_id_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=non_member_user.id)
        with pytest.raises(PermissionDenied) as exc_info:
            await get_checklist_by_id_service.execute(
                access_token, sample_checklist.id.value
            )
        assert "You do not have access to this checklist" in str(exc_info.value)

    async def test_get_checklist_by_id_not_found(
        self, owner_user, get_checklist_by_id_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        with pytest.raises(CheckListNotFoundError):
            await get_checklist_by_id_service.execute(access_token, ID().value)

    async def test_get_checklist_by_id_invalid_id_format(
        self, owner_user, get_checklist_by_id_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        with pytest.raises(CheckListNotFoundError):
            await get_checklist_by_id_service.execute(access_token, "invalid-id-format")
