...
class TestAddAssignee:
    ...
    async def test_add_assignee_success_by_owner(
        ...
        assignees = await add_assignee_service.uow.cards.get_card_assignee_IDs(
            sample_card.id
        )
        assert len(assignees) == 1
        assert assignees[0].value == assignee_user.id.value

    async def test_add_assignee_success_by_admin(
        self, admin_user, assignee_user, sample_card, add_assignee_service, encoder
    ):
    ...
    async def test_add_assignee_permission_denied_for_member(
        self, member_user, assignee_user, sample_card, add_assignee_service, encoder
    ):
        ...
        assert "Only owner or admin can add assignees" in str(exc_info.value)

    async def test_add_assignee_permission_denied_for_non_member(
        self, non_member_user, assignee_user, sample_card, add_assignee_service, encoder
    ):
    ...
    async def test_add_assignee_card_not_found(
        self, owner_user, assignee_user, add_assignee_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
    ...
    async def test_add_assignee_assignee_user_not_found(
        self, owner_user, sample_card, add_assignee_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        with pytest.raises(UserNotFoundError):
            await add_assignee_service.execute(
                access_token, sample_card.id.value, ID().value
            )

    async def test_add_assignee_duplicate_no_error(
        self, owner_user, assignee_user, sample_card, add_assignee_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        # First addition
        ...
        # Second addition – should not raise
        await add_assignee_service.execute(
            access_token, sample_card.id.value, assignee_user.id.value
        )
        ...
        assert len(assignees) == 1  # Still only one assignee

    async def test_add_assignee_multiple_assignees(
        self, owner_user, assignee_user, sample_card, add_assignee_service, encoder, uow
    ):
        # Create a second assignee user
        second_assignee = UserFactory.create(
            email="assignee2@example.com", hashed_password="hashed"
        )
        await uow.users.add(second_assignee)
        await uow.commit()
        ...
        await add_assignee_service.execute(
            access_token, sample_card.id.value, second_assignee.id.value
        )
        ...
        assignee_ids = {a.value for a in assignees}
        assert assignee_user.id.value in assignee_ids
        assert second_assignee.id.value in assignee_ids
