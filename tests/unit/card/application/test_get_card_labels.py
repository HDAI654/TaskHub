...
class TestGetCardLabels:
    ...
    @pytest.fixture
    async def get_card_labels_service(self, uow, token_repo, decoder):
        return GetCardLabelsService(uow, token_repo, decoder)
    ...
    async def test_get_card_labels_success_by_owner(
        self,
        owner_user,
        sample_card,
        add_label_service,
        get_card_labels_service,
        encoder,
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        await add_label_service.execute(access_token, sample_card.id.value, "bug")
        await add_label_service.execute(access_token, sample_card.id.value, "feature")

        labels = await get_card_labels_service.execute(
            access_token, sample_card.id.value
        )
        assert len(labels) == 2
        label_names = {l.name.value for l in labels}
        assert label_names == {"bug", "feature"}

    async def test_get_card_labels_success_by_admin(
        self,
        admin_user,
        sample_card,
        add_label_service,
        ...
        await add_label_service.execute(
            access_token, sample_card.id.value, "admin-label"
        )
        ...
        assert len(labels) == 1
        assert labels[0].name.value == "admin-label"

    async def test_get_card_labels_success_by_member(
        self, member_user, sample_card, get_card_labels_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=member_user.id)
        labels = await get_card_labels_service.execute(
            access_token, sample_card.id.value
        )
        # Member can view labels (even if empty)
        assert isinstance(labels, list)

    async def test_get_card_labels_success_by_viewer(
        self, viewer_user, sample_card, get_card_labels_service, encoder
    ):
    ...
    async def test_get_card_labels_permission_denied_for_non_member(
        self, non_member_user, sample_card, get_card_labels_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=non_member_user.id)
        with pytest.raises(PermissionDenied) as exc_info:
            await get_card_labels_service.execute(access_token, sample_card.id.value)
        assert "You do not have access to this card" in str(exc_info.value)

    async def test_get_card_labels_empty(
        self, owner_user, sample_card, get_card_labels_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        labels = await get_card_labels_service.execute(
            access_token, sample_card.id.value
    ...
    async def test_get_card_labels_card_not_found(
        self, owner_user, get_card_labels_service, encoder
    ):
        access_token = encoder.create_access_token(user_id=owner_user.id)
        with pytest.raises(CardNotFoundError):
            await get_card_labels_service.execute(access_token, ID().value)

    async def test_get_card_labels_multiple_cards_isolation(
        self,
        owner_user,
        sample_card,
        add_label_service,
        ...
        # Add labels to both cards
        await add_label_service.execute(
            access_token, sample_card.id.value, "card1-label"
        )
        await add_label_service.execute(access_token, card2.id.value, "card2-label")

        # Get labels for first card
        labels1 = await get_card_labels_service.execute(
            access_token, sample_card.id.value
        )
        assert len(labels1) == 1
        assert labels1[0].name.value == "card1-label"

        # Get labels for second card
        labels2 = await get_card_labels_service.execute(access_token, card2.id.value)
        assert len(labels2) == 1
        assert labels2[0].name.value == "card2-label"
